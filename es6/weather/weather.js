...
angular.module("asafdav.ngWeather", [])
	.value("apiEndpoint", "http://api.openweathermap.org/data/2.5/weather?id=1862415")
    ...
    this.q = function(d, f) { 
        ...
        var h = c + "?q=" + d + "&units=" + f + "&callback=JSON_CALLBACK"; 
        return a.jsonp(h)
        	.success(function(a) { g.resolve(new e(a)) })
        	.error(function(a) { g.reject(a) }), g.promise } }])
	.directive("cityWeather", ["weatherService", function(a) { 
		return { 
			scope: { cityWeather: "&" }, 
			restrict: "A", 
			template: '<div class="weather-widget"><h1>Temperature in {{city}} <img ng-src="{{weather.getIcon()}}" /></h1><h3>Current: {{weather.getTemperature()}}&deg;C</h3><h4>Min: {{weather.getMinTemperature()}}&deg;C, Max: {{weather.getMaxTemperature()}}&deg;C </h4></div>', 
			link: function(b) { 
				b.$watch("cityWeather", function() { 
					b.city = b.$eval(b.cityWeather), 
					a.q(b.city).then(function(a) { b.weather = a }, 
						function() { b.weather = null }) }) 
			} 
		} 
	}]);